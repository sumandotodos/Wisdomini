...
public class VotationController_mono : Task {
	...
	public void startVotationController_mono(Task w, string hostLogin) {
		fader.Start ();
		foreach (UIScaleFader s in sphereScaler) {
			s.Start ();
			s.scaleOutImmediately ();
		}
		votationHostLogin = hostLogin;
		waitFader.Start ();
	...
	public void networkClickOnValue(int playerWho, int value) {
		...
		++nVotes;
		if (nVotes == (GameController_mono.MaxPlayers - 1)) {
			// extract lowest vote
			int minVal = 1000;
			int minIndex = -1;
			bool unique = true;
			for (int j = 0; j < GameController_mono.MaxPlayers; ++j) {
