...
public class VolcanoActivityController_mono : Task {
	...
	public void startVolcanoTask(Task w) {
		...
		GameObject mc = GameObject.Find ("MasterController");
		MasterController_mono mc_normal = mc.GetComponent<MasterController_mono>();
		MasterController_kidsmono mc_kids = mc.GetComponent<MasterController_kidsmono> ();
...
//		
	...
	{	
		if (state == 0) { // keeping an eye on unanimity
			...
			for (int i = firstPlayerPresent; i < GameController_mono.MaxPlayers; ++i) 
			{
