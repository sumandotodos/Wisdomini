...
public class Rosetta : MonoBehaviour {
	...
	public void addTranslation(string name) {
		RosettaHashDict newDict = new RosettaHashDict ();
		newDict.localeName = name;
		newDict.initialize (firstLevelHashSize, secondLevelHashSize);
		data.Add (newDict);
	}
	...
	public void registerString(string key, string text) {
		...
		ListRosettaDictElement1D targetList = data[selectedTranslation].data.theList[fl].theList[sl];
		...
		if (found) { // update only text
			...
			if(selectedTranslation==0) ++nStrings;
	...
	public string retrieveString(string key) {
		...
		selectedTranslation = currentTranslationIndex;
		currentTranslation = data[selectedTranslation];

		//currentTranslation = defaultTranslation;

		bool foundInTranslation = false;
		ListRosettaDictElement1D targetList = null;
		string result = "";
		if (currentTranslation.data.theList.Count > fl && currentTranslation.data.theList [fl].theList.Count > sl) {
		...
		if (!foundInTranslation) { // return default text
