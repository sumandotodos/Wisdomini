...
public class LifeTestActivityController_multi : Task, LongTouchListener {
	...
	public VotationController_multi votationController;
	...
	public ExplainController_multi explainController;
	...
	public LifeTestVoteResultController_multi resultController;
...
//	}
	...
	void init(int forceSabiduria, int forceSabio, int forceQuestion) {
		...
		if (mode == NormalMode) {
			QuestionNumber = questionNumber = Random.Range (0, testBank.nItems ());
		}
	...
	public void startLifeTestActivity(Task w, int sabid, int sbio, int q, bool turnPlayer) {
		ScreenState = 0;
...
//					clickedHero = 5;
		...
		// waiting for fadeout. We got wisdom and individual right.
			...
			if (!isWaitingForTaskToComplete) {
				...
				resultController.startLifeTestVoteResultController_multi (this);
				gameController.networkAgent.broadcast ("startlifetestvote:" + categoryAnswer + ":" + individualAnswer + ":");
				state2 = 9;
...
//				if (activityResult == 1) { // we won 
//					notifyFinishTask ();
//					state2 = 0;
//					if (mode == SummonMode) {
//						gameController.networkAgent.sendCommand (gameController.playerList [SummoningPlayer].login, "summonres:1:");
//					}
//				} else { // we lost
//					if (mode == SummonMode) {
//						notifyFinishTask ();
//						state2 = 0;
//						gameController.networkAgent.sendCommand (gameController.playerList [SummoningPlayer].login, "summonres:0:");
...
//							"", "");
//					} else {
//						state2 = 3000; // have a last chance
//					}
//				}
		...
		// wait for last chance to finish
			...
			if (!isWaitingForTaskToComplete) {
				if (lastChanceResult == LastChanceController_multi.DEFEAT) {
					notifyFinishTask ();
					MusicController.fadeOut ();
					state2 = 0;
				} else {
		...
		// waiting for screen touch
			...
			if (Input.GetMouseButtonUp (0)) {
				if ((touchCoords - Input.mousePosition).magnitude < 1.0f) {
					fader.fadeOutTask (this);
					if (mode == SummonMode) {
						gameController.networkAgent.sendCommand (gameController.playerList [SummoningPlayer].id, "summonres:0:");
					}
					state2 = 14;
				}
			}
...
//		startLifeTestActivity (this, LifeTestActivityController_multi.SummonMode, sbio, sria, q, sPlayer);
//	}
	...
	public void startNonTurnLifeTest(int sabid, int sabio, int question) {
		...
		rakingCanvas.SetActive (false);
		lifeTestAllCanvas.SetActive (true);
		startLifeTestActivity (this, sabid, sabio, question, false);
	}
